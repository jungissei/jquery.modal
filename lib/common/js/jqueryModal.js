...
; (function (factory) {
  ...
  Modal.prototype.add_modal_open_event = function () {
    ...
    _.$open_btn.on('click', function (e) {
      ...
      let $clicked_opened_btn = $(e.target);

      _.modal_job['open'].bind(_)($clicked_opened_btn);
    });
  };
   ...
   * モーダルが開く直前
   */
  Modal.prototype.before_open = function () {
    let _ = this;

    _.$canvas.on('modal.before_open', function () {
   ...
   * モーダルが閉じる直前
   */
  Modal.prototype.before_close = function () {
    let _ = this;

    _.$canvas.on('modal.before_close', function () {
