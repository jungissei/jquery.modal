...
// ----------------------------------------------------------------------------
{
  ...
  function get_hamburger_menu_nav_html(contents_array, ul_class, id = '') {
    ...
    html += '<ul';
    if (id !== '') html += ' id="' + id + '"';
    html += ' class="' + ul_class + '">' + li_html + '</ul>';
  ...
  function hamburger_menu_nav_item_html_start_tag(index, item) {
    ...
    if (item.url === void 0) {
      start_html += '<button class="item_header" role="button" aria-expanded="false" aria-controls="nav_hamburger_menu' + index + '" tabindex="0">';
    } else {
      start_html += '<a href="' + item.url + '" class="item_header"';
